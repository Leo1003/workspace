...
def search_file(file_name, start_dir = os.getcwd(), stop_dir = None, search_subdir = []):
    """
    ...
    :type stop_dir: str
    :param search_subdir: The sub directories to be also searched in each level.
    :type search_subdir: list[str]
    :rtype: str
    :return: The file path where the file was first found.
    """
    cur_dir = path.abspath(start_dir) if not path.isabs(start_dir) else start_dir

    while True:
        if path.exists(path.join(cur_dir, file_name)):
            # The file of interest exists in the current directory, so return it.
            return path.join(cur_dir, file_name)
        else:
            for subdir in search_subdir:
                if path.exists(path.join(cur_dir, subdir, file_name)):
                    return path.join(cur_dir, subdir, file_name)
        ...
        if parent_dir == cur_dir or parent_dir == stop_dir:
            # We are either at the root directory or reached the stop directory.
            return None
        else:
            cur_dir = parent_dir

def file_exists(file_name, start_dir = os.getcwd(), search_subdir = []):
    """
    ...
    """
    return search_file(file_name, start_dir, None, search_subdir) is not None

def make_path_absolute(file_path, base_dir=os.getcwd()):
    """
    Make a given path absolute using the given base directory if it is
    not already absolute.
    :param file_path: The path of interest.
    :type file_path: str
    ...
    if path.isabs(file_path):
        ...
        return path.join(base_dir, file_path)
...
def make_final_flags(file_name, flags, base_dir = os.getcwd()):
    ...
    if is_cpp_file(file_name):
        final = safe_add_flags(absolute, CPP_FLAGS)
    elif is_c_file(file_name):
        final = safe_add_flags(absolute, C_FLAGS)
    else:
        final = absolute

    logging.info("Final flags: {}".format(final))

    return into_ycm_result(final)

def safe_add_flags(old_flags, additional_flags):
    """
...
def into_ycm_result(flags, do_cache = True, **kwargs):
    """
...
def parse_compile_commands(file_name, search_base):
    """
    ...
    """
    database_path = path.dirname(search_file("compile_commands.json",
            search_base, None, BUILD_DIRECTORIES))
    ...
    if is_header(file_name):
        ...
        for ext in (C_SOURCE_EXTENSIONS + CPP_SOURCE_EXTENSIONS):
            ...
            if path.exists(alternative_name):
...
def parse_clang_complete(file_name, search_base):
    """
    ...
    """
    config = search_file(".clang_complete", search_base)
...
def parse_default_flags(file_name, search_base):
    """
    ...
    """
    logging.debug("Searching additional include directory...")
    flags = BASE_FLAGS
    for incdir in INCLUDE_DIRECTORIES:
        incpath = search_file(incdir, search_base)
        logging.debug("    search_file({}, {})".format(incdir, search_base))
        if incpath is not None:
            logging.debug("Found additional include directory: {}".format(incpath))
            flags += ['-I', incpath]

    logging.debug("Using default flags: {}".format(flags))

    return make_final_flags(file_name, flags)
...
def Settings(**kwargs):
    ...
    search_base = path.dirname(filename)

    logging.info("ycm_extra_conf parameters:")
    logging.info("    Filename: {}".format(filename))
    logging.info("    Language: {}".format(language))
    logging.info("    Base directory: {}".format(search_base))

    if file_exists("compile_commands.json", search_base, BUILD_DIRECTORIES):
        logging.info("Found compile_commands.json, parsing compilation database...")
        return parse_compile_commands(filename, search_base)
    elif file_exists(".clang_complete", search_base):
        logging.info("Found .clang_complete, parsing compilation flags...")
        return parse_clang_complete(filename, search_base)
    else:
        logging.info("Using default flags...")
        return parse_default_flags(filename, search_base)
